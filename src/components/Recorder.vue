        ...
        mediaRecorder.value.onstop = async () => {
            ...
            try {
                ...
                const duration =
                    Date.now() -
                    startTime.value -
                    pausedDurations.reduce((acc, cur) => acc + cur, 0);
                ...
                if (duration > 0) {
                    try {
                        ...
                        console.warn(
                            "Duration fix failed, using original blob:",
                            durationError,
                        );
...
async function startWebcam() {
    ...
    if (webcamSrc.value && webcamContainer.value) {
        try {
            ...
            if (!PIPWINDOW.value) {
                PIPWINDOW.value =
                    await Globalwindow.documentPictureInPicture.requestWindow({
                ...
                const stopButton =
                    PIPWINDOW.value.document.querySelector("#stopButton");
                ...
                if (stopButton) {
                    stopButton.addEventListener("mouseenter", () => {
                        ...
                        stopButton.style.boxShadow =
                            "0 2px 8px rgba(0, 0, 0, 0.2), 0 4px 12px rgba(95, 99, 104, 0.15)";
                    });
...
function addPauseResumeEventListener() {
    ...
    const addEventListenersToButtons = () => {
        const pauseResumeButton =
            PIPWINDOW.value?.document.querySelector("#pauseResumeButton");
        ...
        if (pauseResumeButton) {
            ...
            pauseResumeButton.addEventListener("mouseenter", () => {
                ...
                pauseResumeButton.style.boxShadow =
                    "0 2px 8px rgba(0, 0, 0, 0.2), 0 4px 12px rgba(66, 133, 244, 0.15)";
            });
        ...
        if (stopButtonRecording) {
            ...
            stopButtonRecording.addEventListener("mouseenter", () => {
                ...
                stopButtonRecording.style.boxShadow =
                    "0 2px 8px rgba(0, 0, 0, 0.2), 0 4px 12px rgba(234, 67, 53, 0.15)";
            });

            stopButtonRecording.addEventListener("mouseleave", () => {
                stopButtonRecording.style.background =
                    "rgba(234, 67, 53, 0.12)";
...
const getRecordedVideosFromIndexedDB = async (id?: number) => {
    try {
        ...
        if (id) {
            const data =
                recordedVideos.value.find((dt) => dt.id === id) || null;
...
onMounted(() => {
    ...
    webcamSrc.value?.addEventListener(
        "leavepictureinpicture",
        handleWebcamPiPLeave,
    );
});

onBeforeUnmount(() => {
    webcamSrc.value?.removeEventListener(
        "leavepictureinpicture",
        handleWebcamPiPLeave,
    );
    stopTimer();
...
const containerWebCamStyle =
    "display: flex; flex-direction: column; gap: 20px; align-items: center; padding: 16px;";
const webcamStyle =
    "width: 300px; height: 200px; border-radius: 0.75rem; object-fit: cover; border: 2px solid rgba(255, 255, 255, 0.15); box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.1);";
...
const buttonContainerStyle =
    "display: flex; align-items: center; justify-content: center; gap: 4px; background: rgba(255, 255, 255, 0.08); backdrop-filter: blur(24px) saturate(200%); -webkit-backdrop-filter: blur(24px) saturate(200%); border-radius: 12px; padding: 4px; border: 1px solid rgba(255, 255, 255, 0.08); box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);";
...
<template>
    <div
        class="relative flex-col items-start gap-8 md:flex md:w-[350px]"
        :class="{ hidden: !mobile }"
    >
        <div class="flex flex-col w-full items-start gap-6">
            <!-- Quality Info Display -->
            <div
                class="w-full p-3 relative overflow-hidden"
                style="
            ...
            >
                ...
                <div class="relative">
                    <div class="flex items-center gap-2 mb-2">
                        <div
                            class="w-2 h-2 rounded-full bg-gradient-to-r from-green-400 to-blue-500 animate-pulse"
                        ></div>
                        <h4
                            class="text-sm font-medium bg-gradient-to-r from-gray-700 to-gray-900 dark:from-gray-200 dark:to-gray-400 bg-clip-text text-transparent"
                        >
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        ...
                        >
                            ...
                            <span
                                ...
                                >{{ getCurrentQualityInfo().resolution }}</span
                            >
                        </div>
                        <div
                            class="flex items-center justify-between p-2 rounded-lg"
                        ...
                        >
                            ...
                            <span
                                class="font-medium text-foreground/90 text-xs"
                                >{{ getCurrentQualityInfo().fps }}</span
                            >
                        </div>
                        <div
                            class="flex items-center justify-between p-2 rounded-lg"
                        ...
                        >
                            ...
                            <span
                                class="font-medium text-foreground/90 text-xs"
                                >{{ getCurrentQualityInfo().bitrate }}</span
                            >
                        </div>
                        <div
                            class="flex items-center justify-between p-2 rounded-lg"
                        ...
                        >
                            ...
                            <span
                                class="font-medium text-foreground/90 text-xs"
                                >{{ getCurrentQualityInfo().codec }}</span
                            >
                        </div>
                    </div>

                    <div
                        class="mt-2 flex items-center justify-between p-2 rounded-lg text-xs"
                        style="
                    ...
                    >
                        ...
                        <span
                            ...
                            >{{ getCurrentQualityInfo().mode }}</span
                        >
                    </div>
                </div>
            </div>
            ...
            <div class="w-full space-y-4">
                <!-- Recording Controls -->
                <div
                    class="p-3 relative overflow-hidden"
                    style="
                        background: rgba(255, 255, 255, 0.08);
                        backdrop-filter: blur(20px) saturate(180%);
                        -webkit-backdrop-filter: blur(20px) saturate(180%);
                ...
                >
                    <!-- Decorative gradient -->
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-red-500/5 via-transparent to-blue-500/5 pointer-events-none"
                    ></div>
                    ...
                    <div class="relative">
                        <template v-if="!isRecording">
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger as-child>
                                                <button
                                                    ...
                                                    class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gradient-to-r from-green-400 to-green-600 text-white font-semibold rounded-[10px] transition-all duration-300"
                                                >
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            <TooltipTrigger as-child>
                                                <button
                                                    @click="
                                                        startRecordingWithAudioMic()
                                                    "
                                                    class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gradient-to-r from-blue-400 to-blue-600 text-white font-semibold rounded-[10px] transition-all duration-300"
                                                >
                            ...
                            <button
                                v-if="mobile"
                                @click="startRecordingOnlyAudioMic()"
                                class="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm bg-gradient-to-r from-blue-400 to-blue-600 text-white font-semibold rounded-[10px] transition-all duration-300"
                            >
                                <Mic class="size-4"></Mic>
                        ...
                        <template v-if="isRecording">
                            <div class="flex gap-2">
                                <button
                                    @click="stopRecording()"
                                    class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gradient-to-r from-red-400 to-red-600 text-white font-semibold rounded-[10px] transition-all duration-300"
                                    :class="{
                                ...
                                <button
                                    ...
                                    class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gradient-to-r from-yellow-400 to-orange-500 text-white font-semibold rounded-[10px] transition-all duration-300"
                                >
                                    <Pause
                                        v-if="!isPausedRecord"
                                        class="size-4"
                ...
                <!-- Camera Toggle -->
                <div
                    class="flex items-center gap-3 justify-between p-3 rounded-lg"
                    style="
                        background: rgba(255, 255, 255, 0.08);
                        backdrop-filter: blur(20px) saturate(180%);
                        -webkit-backdrop-filter: blur(20px) saturate(180%);
                ...
                >
                    ...
                    <Switch
                        v-model:checked="enableCameraView"
                        id="enable-camera"
                        :disabled="disableCameraView"
                        @update:checked="toggleWebCamp"
                    />
                </div>
            </div>
            <fieldset
                class="rounded-lg border p-4 w-full"
                style="
                    background: rgba(255, 255, 255, 0.08);
                    backdrop-filter: blur(20px) saturate(180%);
                    -webkit-backdrop-filter: blur(20px) saturate(180%);
                    ...
                    >
                        <div class="flex-1 block text-sm">
                            ...
                            <p class="text-xs text-gray-400">
                                {{ convertBytesAdaptive(video.blob.size || 0) }}
                            </p>
                        </div>
                        <Button
                            size="icon"
                            ...
                            @click.stop="deleteData(video)"
                        >
                            <Trash class="w-4 h-4"></Trash>
                        </Button>
                    </li>
                    <li v-if="recordedVideos.length === 0">No data saved.</li>
                </ul>
            </fieldset>
        </div>
    </div>
    <div
        ref="webcamContainerParent"
        :class="{ '!hidden': !enableCameraView }"
        class="z-[100] fixed bottom-10 right-10"
    >
        <div ref="webcamContainer" :style="containerWebCamStyle">
            ...
            <div style="display: flex; flex-direction: row; gap: 4px">
                ...
                <div :style="buttonContainerStyle">
                    <button
                        v-if="isRecording && recordedType === 'scr_mic'"
                        :style="
                            buttonStopWebcamStyle +
                    ...
                    <button
                        v-else
                        :style="buttonStopWebcamStyle + ' color: white;'"
                    >
                        {{ formatTime(elapsedTime) }}
                    </button>
                </div>
